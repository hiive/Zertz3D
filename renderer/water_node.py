...
class WaterNode:
    ...
    def __init__(self, renderer, x1, y1, x2, y2, z, anim, distort):
        ...
        self.waterNP.setShader(self.renderer.loader.loadShader('shaders/water.sha'))
        ...
        self.water_cam_np.reparentTo(renderer.render)  # reflection camera
        cam = self.water_cam_np.node()
        cam.getLens().setFov(self.renderer.camLens.getFov())
        ...
        cam.setTagState('True', RenderState.make(
            ShaderAttrib.make().setShader(
                self.renderer.loader.loadShader('shaders/splut3Clipped.sha'))))
        """
        ...
        tex1 = self.renderer.loader.loadTexture('models/source.png')
